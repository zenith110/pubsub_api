    ...
    async def not_on_sale(self, context):
        ...
        try:
            for pubsub in range(len(not_on_sale_list)):
                discord_embeds.append(discord.Embed(
                        title=not_on_sale_list[pubsub]["name"],
                        description="Last seen on sale on: " + not_on_sale_list[pubsub]["last_on_sale"] + " for the price of " + not_on_sale_list[pubsub]["price"],
                        color=0x115599,
                    ).set_image(
                        url=not_on_sale_list[pubsub]["image"]
                    ))
