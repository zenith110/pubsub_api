...
// Constants
var CONVERSATION_API_AGENT_VERSION_HEADER = 'Agent-Version-Label';
...
var ActionsSdkApp = function (_AssistantApp) {
  ...
  function ActionsSdkApp(options) {
    _classCallCheck(this, ActionsSdkApp);
    ...
    if (_this.body_ && !_this.isNotApiVersionOne_()) {
      _this.body_ = transformToCamelCase(_this.body_);
    }

    if (_this.body_ && _this.body_.conversation && _this.body_.conversation.type && _this.body_.conversation.type === _this.ConversationStages.NEW && _this.sessionStarted_ && typeof _this.sessionStarted_ === 'function') {
  ...
  _createClass(ActionsSdkApp, [{
    ...
    value: function getRawInput() {
      ...
      var rawInput = input.rawInputs[0];
      if (!rawInput.query) {
    ...
    key: 'getActionVersionLabel',
    value: function getActionVersionLabel() {
      debug('getActionVersionLabel');
      var versionLabel = this.request_.get(CONVERSATION_API_AGENT_VERSION_HEADER);
      if (versionLabel) {
        return versionLabel;
      } else {
        return null;
    ...
    key: 'ask',
    value: function ask(inputPrompt, dialogState) {
      debug('ask: inputPrompt=%s, dialogState=%s', JSON.stringify(inputPrompt), JSON.stringify(dialogState));
      var expectedIntent = this.buildExpectedIntent_(this.StandardIntents.TEXT, []);
      if (!expectedIntent) {
    ...
    key: 'askWithList',
    value: function askWithList(inputPrompt, list, dialogState) {
      debug('askWithList: inputPrompt=%s, list=%s, dialogState=%s', JSON.stringify(inputPrompt), JSON.stringify(list), JSON.stringify(dialogState));
      ...
      var expectedIntent = this.buildExpectedIntent_(this.StandardIntents.OPTION, []);
      if (!expectedIntent) {
        error('Error in building expected intent');
        return null;
      }
      if (this.isNotApiVersionOne_()) {
        expectedIntent.inputValueData = Object.assign(_defineProperty({}, this.ANY_TYPE_PROPERTY_, this.InputValueDataTypes_.OPTION), {
          listSelect: list
        });
      } else {
    ...
    value: function askForDeliveryAddress(reason, dialogState) {
      debug('askForDeliveryAddress: reason=%s', reason);
      if (!reason) {
        this.handleError_('reason cannot be empty');
        return null;
      }
      var expectedIntent = this.buildExpectedIntent_(this.StandardIntents.DELIVERY_ADDRESS, []);
      if (!expectedIntent) {
        error('Error in building expected intent');
        return null;
      }
      expectedIntent.inputValueData = Object.assign(_defineProperty({}, this.ANY_TYPE_PROPERTY_, this.InputValueDataTypes_.DELIVERY_ADDRESS), {
        addressOptions: {
          reason: reason
        }
      });
      var inputPrompt = this.buildInputPrompt(false, 'PLACEHOLDER_FOR_DELIVERY_ADDRESS');
      return this.buildAskHelper_(inputPrompt, [expectedIntent], dialogState);
    }
    ...
    key: 'askWithCarousel',
    value: function askWithCarousel(inputPrompt, carousel, dialogState) {
      debug('askWithCarousel: inputPrompt=%s, carousel=%s, dialogState=%s', JSON.stringify(inputPrompt), JSON.stringify(carousel), JSON.stringify(dialogState));
    ...
    key: 'tell',
    value: function tell(textToSpeech) {
      ...
      if (typeof textToSpeech === 'string') {
        ...
        if (textToSpeech.items) {
          ...
          finalResponse.richResponse = this.buildRichResponse().addSimpleResponse(textToSpeech);
        } else {
          this.handleError_('Invalid speech response. Must be string, ' + 'RichResponse or SimpleResponse.');
          return null;
        }
      }
      var response = this.buildResponseHelper_(null, false, null, finalResponse);
    ...
    key: 'buildInputPrompt',
    value: function buildInputPrompt(isSsml, initialPrompt, noInputs) {
      debug('buildInputPrompt: isSsml=%s, initialPrompt=%s, noInputs=%s', isSsml, initialPrompt, noInputs);
      var initials = [];
    ...
    key: 'buildResponseHelper_',
    value: function buildResponseHelper_(conversationToken, expectUserResponse, expectedInput, finalResponse) {
      debug('buildResponseHelper_: conversationToken=%s, expectUserResponse=%s, ' + 'expectedInput=%s, finalResponse=%s', conversationToken, expectUserResponse, JSON.stringify(expectedInput), JSON.stringify(finalResponse));
      var response = {};
    ...
    key: 'maybeAddItemToArray_',
    value: function maybeAddItemToArray_(item, array) {
    ...
    value: function extractData_() {
      debug('extractData_');
      if (this.body_.conversation && this.body_.conversation.conversationToken) {
        var json = JSON.parse(this.body_.conversation.conversationToken);
        this.data = json.data;
    ...
    value: function fulfillPermissionsRequest_(permissionsSpec, dialogState) {
      debug('fulfillPermissionsRequest_: permissionsSpec=%s, dialogState=%s', JSON.stringify(permissionsSpec), JSON.stringify(dialogState));
      // Build an Expected Intent object.
      var expectedIntent = {
        intent: this.StandardIntents.PERMISSION
      };
      if (this.isNotApiVersionOne_()) {
        expectedIntent.inputValueData = Object.assign(_defineProperty({}, this.ANY_TYPE_PROPERTY_, this.InputValueDataTypes_.PERMISSION), permissionsSpec);
      } else {
        expectedIntent.inputValueSpec = {
          permissionValueSpec: permissionsSpec
        };
      }
      var inputPrompt = this.buildInputPrompt(false, 'PLACEHOLDER_FOR_PERMISSION');
      if (!dialogState) {
        dialogState = {
    ...
    value: function fulfillTransactionRequirementsCheck_(transactionRequirementsSpec, dialogState) {
      debug('fulfillTransactionRequirementsCheck_: transactionRequirementsSpec=%s,' + ' dialogState=%s', JSON.stringify(transactionRequirementsSpec), JSON.stringify(dialogState));
      // Build an Expected Intent object.
      var expectedIntent = {
        intent: this.StandardIntents.TRANSACTION_REQUIREMENTS_CHECK
      };
      expectedIntent.inputValueData = Object.assign(_defineProperty({}, this.ANY_TYPE_PROPERTY_, this.InputValueDataTypes_.TRANSACTION_REQ_CHECK), transactionRequirementsSpec);
      var inputPrompt = this.buildInputPrompt(false, 'PLACEHOLDER_FOR_TXN_REQUIREMENTS');
      if (!dialogState) {
        dialogState = {
          'state': this.state instanceof State ? this.state.getName() : this.state,
          'data': this.data
    ...
    value: function fulfillTransactionDecision_(transactionDecisionValueSpec, dialogState) {
      debug('fulfillTransactionDecision_: transactionDecisionValueSpec=%s,' + ' dialogState=%s', JSON.stringify(transactionDecisionValueSpec), JSON.stringify(dialogState));
      // Build an Expected Intent object.
      var expectedIntent = {
        intent: this.StandardIntents.TRANSACTION_DECISION
      };
      expectedIntent.inputValueData = Object.assign(_defineProperty({}, this.ANY_TYPE_PROPERTY_, this.InputValueDataTypes_.TRANSACTION_DECISION), transactionDecisionValueSpec);
      // Send an Ask request to Assistant.
      var inputPrompt = this.buildInputPrompt(false, 'PLACEHOLDER_FOR_TXN_DECISION');
      if (!dialogState) {
        dialogState = {
          'state': this.state instanceof State ? this.state.getName() : this.state,
          'data': this.data
    ...
    value: function fulfillConfirmationRequest_(confirmationValueSpec, dialogState) {
      debug('fulfillConfirmationRequest_: confirmationValueSpec=%s,' + ' dialogState=%s', JSON.stringify(confirmationValueSpec), JSON.stringify(dialogState));
      // Build an Expected Intent object.
      var expectedIntent = {
        intent: this.StandardIntents.CONFIRMATION
      };
      expectedIntent.inputValueData = Object.assign(_defineProperty({}, this.ANY_TYPE_PROPERTY_, this.InputValueDataTypes_.CONFIRMATION), confirmationValueSpec);
      // Send an Ask request to Assistant.
      var inputPrompt = this.buildInputPrompt(false, 'PLACEHOLDER_FOR_CONFIRMATION');
      if (!dialogState) {
        dialogState = {
          'state': this.state instanceof State ? this.state.getName() : this.state,
          'data': this.data
    ...
    value: function fulfillDateTimeRequest_(dateTimeValueSpec, dialogState) {
      debug('fulfillDateTimeRequest_: dateTimeValueSpec=%s,' + ' dialogState=%s', JSON.stringify(dateTimeValueSpec), JSON.stringify(dialogState));
      // Build an Expected Intent object.
      var expectedIntent = {
        intent: this.StandardIntents.DATETIME
      };
      expectedIntent.inputValueData = Object.assign(_defineProperty({}, this.ANY_TYPE_PROPERTY_, this.InputValueDataTypes_.DATETIME), dateTimeValueSpec);
      // Send an Ask request to Assistant.
      var inputPrompt = this.buildInputPrompt(false, 'PLACEHOLDER_FOR_DATETIME');
      if (!dialogState) {
        dialogState = {
          'state': this.state instanceof State ? this.state.getName() : this.state,
          'data': this.data
    ...
    value: function fulfillSignInRequest_(dialogState) {
      ...
      // Send an Ask request to Assistant.
      var inputPrompt = this.buildInputPrompt(false, 'PLACEHOLDER_FOR_SIGN_IN');
      if (!dialogState) {
        dialogState = {
          'state': this.state instanceof State ? this.state.getName() : this.state,
          'data': this.data
    ...
    key: 'buildAskHelper_',
    value: function buildAskHelper_(inputPrompt, possibleIntents, dialogState) {
      debug('buildAskHelper_: inputPrompt=%s, possibleIntents=%s,  dialogState=%s', inputPrompt, possibleIntents, JSON.stringify(dialogState));
      if (!inputPrompt) {
      ...
      if (typeof inputPrompt === 'string') {
        ...
        if (inputPrompt.speech) {
          inputPrompt = { richInitialPrompt: this.buildRichResponse().addSimpleResponse(inputPrompt) };
      ...
      var response = this.buildResponseHelper_(JSON.stringify(dialogState), true, // expectedUserResponse
      expectedInputs, null // finalResponse is null b/c dialog is active
      );
      return this.doResponse_(response, RESPONSE_CODE_OK);
    }
    ...
    key: 'buildExpectedIntent_',
    value: function buildExpectedIntent_(intent) {
