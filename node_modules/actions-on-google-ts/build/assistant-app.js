...
var RichResponse = require('./response-builder').RichResponse;
var BasicCard = require('./response-builder').BasicCard;
var List = require('./response-builder').List;
var Carousel = require('./response-builder').Carousel;
var OptionItem = require('./response-builder').OptionItem;
var isSsml = require('./response-builder').isSsml;

// Transaction classes
var TransactionValues = require('./transactions').TransactionValues;
var Order = require('./transactions').Order;
var Cart = require('./transactions').Cart;
var LineItem = require('./transactions').LineItem;
var OrderUpdate = require('./transactions').OrderUpdate;

var transformToSnakeCase = require('./utils/transform').transformToSnakeCase;

// Constants
var ERROR_MESSAGE = 'Sorry, I am unable to process your request.';
var API_ERROR_MESSAGE_PREFIX = 'Action Error: ';
var CONVERSATION_API_VERSION_HEADER = 'Google-Assistant-API-Version';
var ACTIONS_CONVERSATION_API_VERSION_HEADER = 'Google-Actions-API-Version';
var ACTIONS_CONVERSATION_API_VERSION_TWO = 2;
var RESPONSE_CODE_OK = 200;
var RESPONSE_CODE_BAD_REQUEST = 400;
var HTTP_CONTENT_TYPE_HEADER = 'Content-Type';
var HTTP_CONTENT_TYPE_JSON = 'application/json';
...
var AssistantApp = function () {
   ...
   */
  function AssistantApp(options, requestData) {
    _classCallCheck(this, AssistantApp);
    ...
    if (this.body_.originalRequest && this.body_.originalRequest.version) {
  ...
  _createClass(AssistantApp, [{
    key: 'handleRequest',
    value: function handleRequest(handler) {
      var _this = this;
      ...
      if (typeof handler === 'function') {
        ...
        var promise = handler(this);
        if (promise instanceof Promise) {
          promise.then(function (result) {
            debug(result);
          }).catch(function (reason) {
            _this.handleError_('function failed: %s', reason.message);
            _this.tell(!reason.message ? ERROR_MESSAGE : reason.message);
          });
        } else {
          // Handle functions
          return;
        ...
        var result = this.invokeIntentHandler_(handler, intent);
        if (!result) {
      ...
      this.handleError_('invalid intent handler type: ' + (typeof handler === 'undefined' ? 'undefined' : _typeof(handler)));
    ...
    value: function askForPermissions(context, permissions, dialogState) {
      debug('askForPermissions: context=%s, permissions=%s, dialogState=%s', context, permissions, JSON.stringify(dialogState));
      if (!context || context === '') {
      ...
      for (var i = 0; i < permissions.length; i++) {
        ...
        if (permission !== this.SupportedPermissions.NAME && permission !== this.SupportedPermissions.DEVICE_PRECISE_LOCATION && permission !== this.SupportedPermissions.DEVICE_COARSE_LOCATION) {
          this.handleError_('Assistant permission must be one of ' + '[NAME, DEVICE_PRECISE_LOCATION, DEVICE_COARSE_LOCATION]');
          return null;
        }
      }
      if (!dialogState) {
    ...
    key: 'askForTransactionRequirements',
    value: function askForTransactionRequirements(transactionConfig, dialogState) {
      debug('checkForTransactionRequirements: transactionConfig=%s,' + ' dialogState=%s', JSON.stringify(transactionConfig), JSON.stringify(dialogState));
      if (transactionConfig && transactionConfig.type && transactionConfig.cardNetworks) {
        this.handleError_('Invalid transaction configuration. Must be of type' + 'ActionPaymentTransactionConfig or GooglePaymentTransactionConfig');
        return null;
      }
      var transactionRequirementsCheckSpec = {};
      ...
      if (transactionConfig && (transactionConfig.type || transactionConfig.cardNetworks)) {
        transactionRequirementsCheckSpec.paymentOptions = this.buildPaymentOptions_(transactionConfig);
      }
      return this.fulfillTransactionRequirementsCheck_(transactionRequirementsCheckSpec, dialogState);
    }
    ...
    key: 'askForTransactionDecision',
    value: function askForTransactionDecision(order, transactionConfig, dialogState) {
      debug('askForTransactionDecision: order=%s, transactionConfig=%s,' + ' dialogState=%s', JSON.stringify(order), JSON.stringify(transactionConfig), JSON.stringify(dialogState));
      if (!order) {
      ...
      var transactionDecisionValueSpec = {
        proposedOrder: order
      };
      if (transactionConfig && transactionConfig.deliveryAddressRequired) {
      ...
      if (transactionConfig && (transactionConfig.type || transactionConfig.cardNetworks)) {
        transactionDecisionValueSpec.paymentOptions = this.buildPaymentOptions_(transactionConfig);
      }
      if (transactionConfig && transactionConfig.customerInfoOptions) {
        if (!transactionDecisionValueSpec.orderOptions) {
          transactionDecisionValueSpec.orderOptions = {};
        }
        transactionDecisionValueSpec.orderOptions.customerInfoOptions = transactionConfig.customerInfoOptions;
      }
      return this.fulfillTransactionDecision_(transactionDecisionValueSpec, dialogState);
    }
    ...
    value: function askForPermission(context, permission, dialogState) {
    ...
    key: 'askForConfirmation',
    value: function askForConfirmation(prompt, dialogState) {
      debug('askForConfirmation: prompt=%s, dialogState=%s', prompt, JSON.stringify(dialogState));
      var confirmationValueSpec = {};
      if (prompt) {
    ...
    key: 'askForDateTime',
    value: function askForDateTime(initialPrompt, datePrompt, timePrompt, dialogState) {
      debug('askForConfirmation: initialPrompt=%s, datePrompt=%s, ' + 'timePrompt=%s, dialogState=%s', initialPrompt, datePrompt, timePrompt, JSON.stringify(dialogState));
      var confirmationValueSpec = {};
    ...
    key: 'askForSignIn',
    value: function askForSignIn(dialogState) {
    ...
    */
    ...
    value: function getUser() {
      ...
      var user = Object.assign({}, requestUser);
    ...
    value: function getUserLocale() {
      debug('getUserLocale');
      return this.getUser() && this.getUser().locale ? this.getUser().locale : null;
    }
    ...
    value: function getDeviceLocation() {
      ...
      var deviceLocation = Object.assign({}, data.device.location);
    ...
    value: function getInputType() {
      ...
      if (data && data.inputs) {
        ...
        try {
          for (var _iterator = data.inputs[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
            ...
            if (input.rawInputs) {
              ...
              try {
                for (var _iterator2 = input.rawInputs[Symbol.iterator](), _step2; !(_iteratorNormalCompletion2 = (_step2 = _iterator2.next()).done); _iteratorNormalCompletion2 = true) {
                  var rawInput = _step2.value;
    ...
    key: 'getArgumentCommon',
    value: function getArgumentCommon(argName) {
      debug('getArgument: argName=%s', argName);
      if (!argName) {
        error('Invalid argument name');
        return null;
      }
      var argument = this.findArgument_(argName);
      if (!argument) {
    ...
    value: function getTransactionRequirementsResult() {
      ...
      var argument = this.findArgument_(this.BuiltInArgNames.TRANSACTION_REQ_CHECK_RESULT);
    ...
    value: function getDeliveryAddress() {
      ...
      var _BuiltInArgNames = this.BuiltInArgNames,
          DELIVERY_ADDRESS_VALUE = _BuiltInArgNames.DELIVERY_ADDRESS_VALUE,
          TRANSACTION_DECISION_VALUE = _BuiltInArgNames.TRANSACTION_DECISION_VALUE;

      var argument = this.findArgument_(DELIVERY_ADDRESS_VALUE, TRANSACTION_DECISION_VALUE);
      if (argument && argument.extension) {
        if (argument.extension.userDecision === this.Transactions.DeliveryAddressDecision.ACCEPTED) {
          var location = argument.extension.location;
    ...
    value: function getTransactionDecision() {
      ...
      var argument = this.findArgument_(this.BuiltInArgNames.TRANSACTION_DECISION_VALUE);
      if (argument && argument.extension) {
    ...
    value: function getUserConfirmation() {
      ...
      var argument = this.findArgument_(this.BuiltInArgNames.CONFIRMATION);
      if (argument) {
    ...
    value: function getDateTime() {
      debug('getDateTime');
      var argument = this.findArgument_(this.BuiltInArgNames.DATETIME);
      if (argument) {
    ...
    value: function getSignInStatus() {
      ...
      var argument = this.findArgument_(this.BuiltInArgNames.SIGN_IN);
    ...
    key: 'hasSurfaceCapability',
    value: function hasSurfaceCapability(requestedCapability) {
      debug('hasSurfaceCapability: requestedCapability=%s', requestedCapability);
      var capabilities = this.getSurfaceCapabilities();
      if (!capabilities) {
        error('No incoming capabilities to search ' + 'for request capability: %s', requestedCapability);
        return false;
      }
    ...
    value: function getSurfaceCapabilities() {
      ...
      if (data && data.surface && data.surface.capabilities) {
        return data.surface.capabilities.map(function (capability) {
    ...
    key: 'buildRichResponse',
    value: function buildRichResponse(richResponse) {
    ...
    key: 'buildBasicCard',
    value: function buildBasicCard(bodyText) {
    ...
    key: 'buildList',
    value: function buildList(title) {
    ...
    key: 'buildOptionItem',
    value: function buildOptionItem(key, synonyms) {
    ...
    key: 'buildOrder',
    value: function buildOrder(orderId) {
    ...
    key: 'buildCart',
    value: function buildCart(cartId) {
    ...
    key: 'buildLineItem',
    value: function buildLineItem(name, id) {
    ...
    key: 'buildOrderUpdate',
    value: function buildOrderUpdate(orderId, isGoogleOrderId) {
    ...
    key: 'invokeIntentHandler_',
    value: function invokeIntentHandler_(handler, intent) {
      var _this2 = this;
      ...
      try {
        for (var _iterator3 = handler.keys()[Symbol.iterator](), _step3; !(_iteratorNormalCompletion3 = (_step3 = _iterator3.next()).done); _iteratorNormalCompletion3 = true) {
          var key = _step3.value;

          var value = handler.get(key);
          var name = void 0;
          ...
          if (name === intent) {
            ...
            var promise = value(this);
            if (promise instanceof Promise) {
              promise.then(function (result) {
                // No-op
              }).catch(function (reason) {
                error(reason.message);
                _this2.handleError_('intent handler failed: %s', reason.message);
                _this2.lastErrorMessage_ = reason.message;
                return false;
              });
            } else {
              // Handle functions
        ...
        try {
          for (var _iterator4 = data.inputs[Symbol.iterator](), _step4; !(_iteratorNormalCompletion4 = (_step4 = _iterator4.next()).done); _iteratorNormalCompletion4 = true) {
            ...
            if (input.arguments) {
              ...
              try {
                for (var _iterator5 = input.arguments[Symbol.iterator](), _step5; !(_iteratorNormalCompletion5 = (_step5 = _iterator5.next()).done); _iteratorNormalCompletion5 = true) {
    ...
    key: 'isNotApiVersionOne_',
    value: function isNotApiVersionOne_() {
      debug('isNotApiVersionOne_');
      return this.actionsApiVersion_ !== null && parseInt(this.actionsApiVersion_, 10) >= ACTIONS_CONVERSATION_API_VERSION_TWO;
    }
    ...
    key: 'handleError_',
    value: function handleError_(text) {
    ...
    key: 'doResponse_',
    value: function doResponse_(response, responseCode) {
      ...
      var httpResponse = this.response_.status(code).send(response);
      this.responded_ = true;
      return httpResponse;
    }
    ...
    value: function fulfillPermissionsRequest_() {
    ...
    value: function fulfillConfirmationRequest_() {
    ...
    value: function fulfillDateTimeRequest_() {
    ...
    value: function fulfillTransactionRequirementsCheck_() {
    ...
    value: function fulfillTransactionDecision_() {
    ...
    key: 'buildPromptsFromSsmlHelper_',
    value: function buildPromptsFromSsmlHelper_(ssmls) {
      debug('buildPromptsFromSsmlHelper_: ssmls=%s', ssmls);
      var prompts = [];
      for (var i = 0; i < ssmls.length; i++) {
        var prompt = {
          ssml: ssmls[i]
        };
        prompts.push(prompt);
    ...
    key: 'buildPromptsFromPlainTextHelper_',
    value: function buildPromptsFromPlainTextHelper_(plainTexts) {
    ...
    key: 'buildPaymentOptions_',
    value: function buildPaymentOptions_(transactionConfig) {
      debug('buildPromptsFromPlainTextHelper_: transactionConfig=%s', JSON.stringify(transactionConfig));
      var paymentOptions = {};
      if (transactionConfig.type) {
        // Action payment
